...
class CanvasUI {
  constructor(content, config) {
    ...
    this.panelSize =
      this.config.panelSize !== undefined ? this.config.panelSize : { width: 1, height: 1 }
        ...
        if (elm.type == 'button') {
          ...
          if (self.keyboard) {
            if (self.keyboard.visible) {
              ...
              const y =
                (0.5 -
                  (elm.position.y + elm.height + self.config.body.padding) / self.config.height) *
  ...
  update() {
    ...
    Object.entries(this.content).forEach(([name, content]) => {
      ...
      if (display !== 'none') {
        ...
        const hover =
          (self.selectedElements[0] !== undefined && this.selectedElements[0] === config) ||
          (self.selectedElements[1] !== undefined && this.selectedElements[1] === config)
  ...
  wrapText(name, txt) {
    ...
    if (textHeight > rect.height && config.overflow === 'scroll') {
      ...
      const fontColor =
        config.fontColor !== undefined ? config.fontColor : this.config.body.fontColor
